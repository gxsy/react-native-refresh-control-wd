...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    private int oldPushDistance = 0;
    private int pushDistance = 0;

    private CircleImageView defaultProgressView = null;
    ...
    private void updateListenerCallBack() {
        ...
        if (!usingDefaultHeader) {
            // 设置视图一起滚动
            if (mHeadViewContainer.getBottom() > mTarget.getTop()) {
                mTarget.offsetTopAndBottom(mHeadViewContainer.getBottom() - mTarget.getTop());
            } else if (mHeadViewContainer.getBottom() < mTarget.getTop()) {
                mTarget.offsetTopAndBottom(-(mTarget.getTop() - mHeadViewContainer.getBottom()));
            }
        }
        if (mListener != null) {
    ...
    public SuperSwipeRefreshLayout(Context context, AttributeSet attrs) {
        ...
        defaultProgressView = createProgressView();
    ...
    public void setDistanceToTriggerSync(int distance) {
        final DisplayMetrics metrics = getResources().getDisplayMetrics();
        mSpinnerFinalOffset = distance * metrics.density;
        mTotalDragDistance = mSpinnerFinalOffset;
    }
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                Log.d(LOG_TAG, "overscrollBottom " + overscrollBottom);
                if (mIsBeingDragged) {
                    pushDistance = (int) overscrollBottom;
                    Log.d(LOG_TAG, "pushDistance 233 " + pushDistance);
    ...
    private void setTargetOffsetTopAndBottom(int offset, boolean requiresUpdate) {
            ...
            Log.d(LOG_TAG, "setTargetOffsetTopAndBottom offset" + offset);
    ...
    private void updateFooterViewPosition() {
        ...
        // 通过记录上次滑动的距离， 拿到相对滚动的距离， 避免在移动时移动的距离很大
        mFooterViewContainer.offsetTopAndBottom(oldPushDistance - pushDistance);
        oldPushDistance = pushDistance;

        updatePushDistanceListener();
    }

    private void updatePushDistanceListener() {
        if (mFooterViewContainer.getTop() < mTarget.getBottom()) {  // 上拉时同步与 footer 的距离
            mTarget.offsetTopAndBottom(-(mTarget.getBottom() - mFooterViewContainer.getTop()));
        } else if (mFooterViewContainer.getTop() > mTarget.getBottom()) { // 移动 footer 时 同步下滑距离
            mTarget.offsetTopAndBottom((mFooterViewContainer.getTop() - mTarget.getBottom()));
        }
        if (mOnPushLoadMoreListener != null) {
            Log.d(LOG_TAG, "pushDistance " + pushDistance);
    ...
    private CircleImageView createProgressView() {
        defaultProgressView = new CircleImageView(getContext(), CIRCLE_BG_LIGHT, CIRCLE_DIAMETER/2);
        mProgress = new MaterialProgressDrawable(getContext(), this);
        mProgress.setBackgroundColor(CIRCLE_BG_LIGHT);
        defaultProgressView.setImageDrawable(mProgress);
    ...
    private Animation startAlphaAnimation(final int startingAlpha, final int endingAlpha) {
        ...
        defaultProgressView.setAnimationListener(null);
        defaultProgressView.clearAnimation();
        defaultProgressView.startAnimation(alpha);
        return alpha;
    }
    ...
    private void moveSpinner(float overscrollTop) {
        ...
        if ( defaultProgressView.getVisibility() != View.VISIBLE) {
            defaultProgressView.setVisibility(View.VISIBLE);
        }
        if (!mScale) {
            ViewCompat.setScaleX(defaultProgressView, 1f);
            ViewCompat.setScaleY(defaultProgressView, 1f);
        }
        if (overscrollTop < mTotalDragDistance) {
            if (mScale) {
                setAnimationProgress(overscrollTop / mTotalDragDistance);
    ...
    public void setProgressBackgroundColorSchemeColor(@ColorInt int color) {
        if (usingDefaultHeader) {
            defaultProgressView.setBackgroundColor(color);
    ...
    public void setSize(int size) {
        ...
        // re-setting it
        defaultProgressView.setImageDrawable(null);
...
//        return defaultProgressView != null ?defaultProgressView.getMeasuredHeight() : 0;
//    }
}
